...
class Player:
    ...
    def action(self):
        ...
        if self.turn == 1 or self.board_representation.player_no_token() == True:
            action = 'THROW'
        elif self.throw_num >= 9:
            action = 'SWING'
        else:
            action = self.choose_random_action()

        if action == 'THROW':
            pos = random.choice(self.board_representation.throw_area(self.throw_num,self.player_type))
            type = random.choice(self.TOKEN_TYPE)
            self.throw_num+= 1
            return self.throw_string(type,pos)

        elif action == 'SWING':
            #choose random token
            original_pos = random.choice(list(itertools.chain(*self.board_representation.get_player_token().values())))
            new_pos = random.choice(self.board_representation.generate_possible_move(original_pos))
            return self.slide_string(original_pos, new_pos)
