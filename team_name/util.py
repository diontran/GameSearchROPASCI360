...
def eval_func(board_state, w):  # sourcery no-metrics
    eval = 0
    h = h1 = h2 = h3 = h4 = h5 = h6 = h7 = h8 = 0
    """ Heuristic to want all enemys defeated"""
    if not board_state['s'] and not board_state['r'] and not board_state['p']:
        h = -100

    """ Heuristic Based on number of total teammates against enemies"""
    h1 = h1 - len(board_state['S']) - len(board_state['R']) - len(board_state['P'])

    h2 = h2 + len(board_state['s']) + len(board_state['r']) + len(board_state['p'])

    """ Heuristic Based on number of total victims against predators"""
    for token_pos in board_state['R']:
        h3 -= token_vps('R', token_pos, board_state)
    for token_pos in board_state['S']:
        h3 -= token_vps('S', token_pos, board_state)
    for token_pos in board_state['P']:
        h3 -= token_vps('P', token_pos, board_state)

    for token_pos in board_state['r']:
        h4 += token_vps('r', token_pos, board_state)
    for token_pos in board_state['s']:
        h4 += token_vps('s', token_pos, board_state)
    for token_pos in board_state['p']:
        h4 += token_vps('p', token_pos, board_state)

    """ Heuristic Based on wellbeing of token"""
    for token_pos in board_state['R']:
        h5 -= token_value('R', token_pos, board_state)
    for token_pos in board_state['S']:
        h5 -= token_value('S', token_pos, board_state)
    for token_pos in board_state['P']:
        h5 -= token_value('P', token_pos, board_state)

    for token_pos in board_state['r']:
        h6 += token_value('r', token_pos, board_state)
    for token_pos in board_state['s']:
        h6 += token_value('s', token_pos, board_state)
    for token_pos in board_state['p']:
        h6 += token_value('p', token_pos, board_state)

    # """ Maybe additional benefits with close teammates as to suggest potential swing moves """
    # for token_pos in board_state['R']:
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['R'])
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['S'])
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['P'])
    # for token_pos in board_state['S']:
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['R'])
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['S'])
    #     h7 += 8 - distance_from_tokens(token_pos, board_state['P'])
    ...
    """Heuristic considering only the closest token to a victim"""
    h8 -= closest_victim('S', board_state)
    h8 -= closest_victim('R', board_state)
    h8 -= closest_victim('P', board_state)

    eval = w[0]*h + w[1]*h1 + w[2]*h2 + w[3]*h3 + w[4]*h4 + w[5]*h5 + w[6]*h8
    return eval

def closest_victim(token_type, board_state):
    distance_list = []
    for token in board_state[token_type]:
        victim_distance = distance_from_tokens(token, get_victims(token_type, board_state))
        distance_list.append(victim_distance)
...
def token_value(token_type, token_pos, board_state):
    ...
    return    (predator_distance - 8) + (8 - victim_distance)

def distance_from_tokens(token_pos, token_list):
    """Returns the total manhattan or euclidean distance from the given token to the list of tokens provided"""
    total = 0
    if token_list != None:
        for pos in token_list:
            total += manhattan_distance(token_pos, pos)
            #total += euclidean_distance(token_pos, pos)
    return total
...
def euclidean_distance(pos1, pos2):
    ...
    return math.sqrt(pos1[0]*pos1[0] + pos2[0]*pos2[0] + pos1[0]*pos2[0])
...
def get_children_states(game_state, upper_throws, lower_throws):
    ...
    for umove in good_upper:
        row = [(umove, lmove) for lmove in good_lower]
...
def create_matrix(original_state, upper_throws, lower_throws):
    ...
    test_3 = [0.1, 1, 1, 0.1, 0.1, 10, 1, 0.5]
    ...
    for lmove in good_lower_moves:
        ...
        for umove in good_upper_moves:
            ...
            matrix_row.append(eval_func(new_state, w_list))
...
def get_player_moves(game_state, player_type, player_throws):
    ...
    #Slides
    keys = ['R','P','S'] if player_type == 'upper' else ['r','p','s']
    empty_board = Board('upper')
    for key in keys:
